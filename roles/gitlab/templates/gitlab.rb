...
external_url 'https://{{ hosts.gitlab.hostname }}.{{ domain.name }}.{{ domain.extension }}'
...
main: # 'main' is the GitLab 'provider ID' of this LDAP server
  label: 'LDAP'
  host: '{{ hosts.ldap.hostname }}.{{ domain.name }}.{{ domain.extension }}'
  port: 389
  uid: 'uid'
  bind_dn: 'dn = uid={{ gitlab.ldap.uid }},dc={{ domain.name }},dc={{ domain.extension }}'
  password: '{{ gitlab.ldap.password }}'
...
###! **Use smtp instead of sendmail/postfix.**
 ...
 gitlab_rails['smtp_address'] = "{{ hosts.mail.hostname }}.{{ domain.name }}.{{ domain.extension }}"
 gitlab_rails['smtp_port'] = 25
# gitlab_rails['smtp_user_name'] = "smtp user"
# gitlab_rails['smtp_password'] = "smtp password"
 gitlab_rails['smtp_domain'] = "{{ domain.name }}.{{ domain.extension }}"
...
# nginx['ssl_verify_depth'] = "1"
 ...
 nginx['ssl_certificate'] = "/etc/ssl/{{ hosts.gitlab.hostname }}/{{ hosts.gitlab.hostname }}.{{ domain.name }}.{{ domain.extension }}.crt"
 nginx['ssl_certificate_key'] = "/etc/ssl/{{ hosts.gitlab.hostname }}/{{ hosts.gitlab.hostname }}.{{ domain.name }}.{{ domain.extension }}.key"
